...
SELECT
	...
	,[ROLE].[ROLE_NAME_LARGE]
	,[STAFF].[BADGE_NUM]
	,[PERSON].[LAST_NAME] + ', ' + [PERSON].[FIRST_NAME] AS [STAFF_NAME]
...
FROM
	...
	INNER JOIN
	rev.[EP_STUDENT_TEAM] AS [SPECIAL_PROGRAMS]
	ON
	[ENROLLMENTS].[STUDENT_GU] = [SPECIAL_PROGRAMS].[STUDENT_GU]
	
	INNER JOIN
	rev.[REV_ROLE] AS [ROLE]
	ON
	[SPECIAL_PROGRAMS].[ROLE_GU] = [ROLE].[ROLE_GU]
	
	LEFT OUTER JOIN
	rev.[EPC_STAFF] AS [STAFF]
	ON
	[SPECIAL_PROGRAMS].[STAFF_GU] = [STAFF].[STAFF_GU]
