...
DECLARE @Grade VARCHAR(2) = '05'
DECLARE @SpedStatus VARCHAR(1) = NULL
...
SELECT 
	...
	,CASE WHEN ISNUMERIC([EPC_STU].[GRID_CODE]) = 0 OR [EPC_STU].[GRID_CODE] = '99999' THEN '' ELSE RIGHT([EPC_STU].[GRID_CODE],3) END AS [GRID_CODE_HS]
...
WHERE
	...
	--[ENROLLMENTS].[ORGANIZATION_GU] LIKE @SchoolGu
	--AND [ENROLLMENTS].[GRADE] = COALESCE (@Grade,[ENROLLMENTS].[GRADE])
	
	--AND [STUDENT].[SPED_STATUS] = COALESCE (@SpedStatus,[STUDENT].[SPED_STATUS])
	
	--AND [ENROLLMENTS].[SCHOOL_CODE] = '465'