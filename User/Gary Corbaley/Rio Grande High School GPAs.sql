...
SELECT
	...
	,[RevYear].[EXTENSION]
FROM
	APS.PrimaryEnrollmentsAsOf('07/25/2014') AS [Enrollments]
...
SELECT --TOP 200
	...
	,CASE WHEN ([ENROLLMENTS].[ENTER_DATE] > '08/14/2014' OR [ENROLLMENTS].[LEAVE_DATE] <= '12/19/2014') AND [ENROLLMENTS].[LEAVE_CODE] != 'WGRAD' THEN 'Y' ELSE 'N' END AS [MOBILITY]
	,[STUDENT].[GENDER]
	,[STUDENT].[RESOLVED_RACE]
	,CASE WHEN [STUDENT].[FRM_CODE] IS NULL THEN 'N' ELSE [STUDENT].[FRM_CODE] END AS [LUNCH]
	,[STUDENT].[ELL_STATUS]
	,[STUDENT].[SPED_STATUS]
	
	--,[GPA].[GPA]
	
	--,[GPA_TYPE].[GPA_TYPE_NAME]
	
	--,[GPA_DEF].[GPA_CODE]
	--,[GPA_DEF].[GPA_TITLE]
	--,[GPA_DEF].[GPA_TYPE]
	--,[GPA_DEF].[ADDITIONAL_FORMULA]
...
FROM
	...
	LEFT OUTER JOIN
	[011-SYNERGYDB.APS.EDU.ACTD].[PR].APS.CumulativeGPA AS [CumulativeGPA]
	ON
	[STUDENT].[SIS_NUMBER] = [CumulativeGPA].[ID_NBR]
	AND [CumulativeGPA].[DST_NBR] = 1
...
WHERE
	...
	[ENROLLMENTS].[SCHOOL_CODE] = '540'
	
	--AND [STU].[USER_CODE_2] LIKE '3%'
	AND [STU].[USER_CODE_2] IS NOT NULL
	
	--AND 
	--[STUDENT].[SIS_NUMBER] = '100008812'
	
	--UPPER([ENROLLMENTS].[SCHOOL_NAME]) LIKE 'RIO%'
