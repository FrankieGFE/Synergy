...
SELECT DISTINCT
	...
	,[ENROLLMENTS].[GRADE]
	,[STUDENT].[BIRTH_DATE]
	,[AZ_IEP].[ST_AST_ALT_YN] AS [ALTERNATIVE ASSESSMENT]
FROM
	...
	ON 
	[STUDENT].[STUDENT_GU] = [IEP].[STUDENT_GU]
	
	INNER JOIN
	rev.[EP_AZ_STUDENT_IEP] AS [AZ_IEP]
	ON
	[IEP].[IEP_GU] = [AZ_IEP].[IEP_GU]
	
WHERE
	[AZ_IEP].[ST_AST_ALT_YN] = 'Y'
