...
FROM
	...
	WHERE
		...
		[RevYear].[SCHOOL_YEAR] = 2014
		AND [RevYear].[EXTENSION] = 'R'
		AND [StudentSchoolYear].[ENTER_DATE] <= '12/19/2014'
		AND '12/19/2014' >= COALESCE([StudentSchoolYear].[LEAVE_DATE],'12/19/2014')
...
SELECT --TOP 200
	...
	,'FALL' AS [SEMESTER]
	,'12/19/2014' AS [SEMESTER_END_DATE]
	,[STUDENT].[SIS_NUMBER]
	,[STUDENT].[STATE_STUDENT_NUMBER]
...
FROM
 ...
 --   AND (
	--	[GPA_TYPE].[GPA_TYPE_NAME] = 'MS Cum Flat' 
	--	OR
	--	[GPA_TYPE].[GPA_TYPE_NAME] = 'MS Year To Date'
	--	)
	----AND [GPA_TYPE].[GPA_TYPE_NAME] IS NOT NULL
