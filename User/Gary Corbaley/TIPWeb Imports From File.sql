...
SELECT
	...
	--,CASE WHEN [CURRENT_SSY].[STUDENT_SCHOOL_YEAR_GU] IS NULL THEN [PREVIOUS_SSY].[STUDENT_SCHOOL_YEAR_GU] ELSE [CURRENT_SSY].[STUDENT_SCHOOL_YEAR_GU] END AS [STUDENT_SCHOOL_YEAR_GU]
	--,(
	--SELECT TOP 1
	...
	--	AND [Lost].[CampusID] = [SSY].[SCHOOL_CODE]
	--	AND (
	--		[Lost].[ModifiedDate] BETWEEN [SSY].[ENTER_DATE] AND [SSY].[LEAVE_DATE]
	--		OR
	--		[Lost].[ModifiedDate] > [SSY].[LEAVE_DATE]
	--		)
	--) AS [STUDENT_SCHOOL_YEAR_GU]
...
FROM
	...
	ON
	[stu].[STUDENT_GU]=[CURRENT_SSY].[STUDENT_GU]
	AND [Lost].[CampusID] = [CURRENT_SSY].[SCHOOL_CODE]
	AND (
	[Lost].[ModifiedDate] BETWEEN [CURRENT_SSY].[ENTER_DATE] AND [CURRENT_SSY].[LEAVE_DATE]
	)
	...
	([Lost].[ModifiedDate] > [PREVIOUS_SSY].[LEAVE_DATE] AND YEAR([Lost].[ModifiedDate]) = YEAR([PREVIOUS_SSY].[LEAVE_DATE]) )
	)*/
...
WHERE
	...
	--AND [Lost].[StudentID] = '103485348'
	--AND [stu].STUDENT_GU = '71FA2CCE-45AA-4DA1-85D3-AE1EA6365FB6'
	--AND [FEECODES_PREVIOUS].[FEE_CODE_GU] IS NULL
...
FROM
	...
	ON
	[stu].[STUDENT_GU]=[PREVIOUS_SSY].[STUDENT_GU]
	AND [Damaged].[CampusID] = [PREVIOUS_SSY].[SCHOOL_CODE]
	AND (
	-- GET PREVIOUS STUDENT SCHOOL YEAR WHERE END DATES ARE IN THE SAME YEAR AS THE DATE FROM THE FILE
	([Damaged].[ModifiedDate] > [PREVIOUS_SSY].[LEAVE_DATE] AND YEAR([Damaged].[ModifiedDate]) = YEAR([PREVIOUS_SSY].[LEAVE_DATE]) AND YEAR([Damaged].[ModifiedDate]) != YEAR([PREVIOUS_SSY].[ENTER_DATE]))
	)*/
