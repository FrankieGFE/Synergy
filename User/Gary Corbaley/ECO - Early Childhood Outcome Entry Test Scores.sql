...
SELECT --TOP 100
	...
	,[ASOF_ENROLLMENTS].[GRADE] AS [GRADE]
	--,[PREV_ENROLLMENTS].[GRADE] AS [PREVIOUS_GRADE]
	--,[ASOF_ENROLLMENTS].[SCHOOL_CODE]
	,[School].[STATE_SCHOOL_CODE]
	,[ASOF_ENROLLMENTS].[SCHOOL_NAME]
	,[ASOF_ENROLLMENTS].[SCHOOL_YEAR]
	,[ASOF_ENROLLMENTS].[ENTER_DATE]
	,[ASOF_ENROLLMENTS].[LEAVE_DATE]
	,[TEST].[TEST_NAME]
	,[TEST].[TEST_DESCRIPTION]
	--,[TEST].[SCHOOL_YEAR]
	,CONVERT(VARCHAR(4),YEAR([STUDENT_TEST].[ADMIN_DATE])) + '-' + RIGHT('000' + CONVERT(VARCHAR(2),DATEPART(MM,[STUDENT_TEST].[ADMIN_DATE])),2) + '-' + RIGHT('000' + CONVERT(VARCHAR(2),DATEPART(DD,[STUDENT_TEST].[ADMIN_DATE])),2) AS [ADMIN_DATE]
...
FROM
	...
	WHERE
		[ENTER_DATE] BETWEEN '07/01/2015' AND '05/25/2016'
		--SCHOOL_YEAR = 2015
		--AND EXTENSION = 'R'
	...
	WHERE
		--[ENTER_DATE] BETWEEN '07/01/2015' AND '06/30/2016'
		SCHOOL_YEAR = 2015
		AND EXTENSION = 'R'
		AND EXCLUDE_ADA_ADM IS NULL
	) AS [PREV_ENROLLMENTS]
	ON
	[ASOF_ENROLLMENTS].[STUDENT_GU] = [PREV_ENROLLMENTS].[STUDENT_GU]
	AND [PREV_ENROLLMENTS].[RN] = 1
	
	LEFT OUTER JOIN 
	rev.EPC_SCH AS [School] -- Contains the School Code / Number
	ON 
	[ASOF_ENROLLMENTS].[ORGANIZATION_GU] = [School].[ORGANIZATION_GU]
...
WHERE
	...
	AND [ASOF_ENROLLMENTS].[RN] = 1
	
	AND [STUDENT_TEST].[ADMIN_DATE] BETWEEN '07/01/2015' AND '05/25/2016'
	
	--AND [STUDENT_DETAILS].[SIS_NUMBER] = '970111138'
