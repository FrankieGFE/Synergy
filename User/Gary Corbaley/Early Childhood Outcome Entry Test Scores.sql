...
SELECT --TOP 100
	...
	,[ASOF_ENROLLMENTS].[SCHOOL_NAME]
	,[TEST].[TEST_NAME]
	,[TEST].[TEST_DESCRIPTION]
	,[TEST].[SCHOOL_YEAR]
	,[STUDENT_TEST].[ADMIN_DATE]
	,[TEST_PART].[PART_NUMBER]
	,[TEST_PART].[PART_DESCRIPTION]
	,[TEST_PART_SCORE].[TEST_SCORE]
...
FROM
	...
	ON
	[STUDENT_DETAILS].[STUDENT_GU] = [STUDENT_TEST].[STUDENT_GU]
	...
	INNER JOIN
	rev.[EPC_TEST_PART] AS [TEST_PART]
	ON
	[STUDENT_TEST].[TEST_GU] = [TEST_PART].[TEST_GU]
	
	INNER JOIN
	rev.[EPC_STU_TEST_PART] AS [STUDENT_TEST_PART]
	ON
	[TEST_PART].[TEST_PART_GU] = [STUDENT_TEST_PART].[TEST_PART_GU]
	AND [STUDENT_TEST].[STUDENT_TEST_GU] = [STUDENT_TEST_PART].[STUDENT_TEST_GU]
	
	INNER JOIN
	rev.[EPC_STU_TEST_PART_SCORE] AS [TEST_PART_SCORE]
	ON
	[STUDENT_TEST_PART].[STU_TEST_PART_GU] = [TEST_PART_SCORE].[STU_TEST_PART_GU]

	--rev.EPC_TEST -- TEST_GU
	--rev.EPC_STU_TEST -- STUDENT_GU TEST_GU
	--rev.EPC_TEST_PART -- TEST_GU
	--rev.EPC_STU_TEST_PART_SCORE -- STU_TEST_PART_GU
	
WHERE
	UPPER([TEST].[TEST_NAME]) LIKE 'EARLY%'
	
	--AND [STUDENT_DETAILS].[SIS_NUMBER] = '970113290'
	
	
--SELECT TOP 100
--	*
--FROM
--	rev.EPC_STU_TEST_PART