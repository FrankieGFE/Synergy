...
SELECT 
	   ...
	   END AS READING
       ,CASE WHEN [ELA/L English 11] != 1 THEN '0'
		     WHEN [ELA/L English 11] IS NOT NULL THEN '0'
			 ELSE [ELA/L English 11]
	   END AS WRITING
FROM
(
...
PIVOT
	(COUNT (SIS_NUMBER) FOR PART_DESCRIPTION IN ([PARCC HS Geometry],[PARCC HS Algebra II],[ELA/L English 11])) AS PARCC_CNT
	   RIGHT JOIN
	   @STUDENTS AS STUDS
	   ON SIS_NUMBER = STUDS.SIS_NUMBER
...
INSERT INTO @EOC_SBA_AND_EOC
	...
	SELECT
		...
		,CAST(WR_CNT.TEST AS INT) + CAST(PARCC_CNTS.WRITING AS INT) AS WRI_ATTEMPTS
...
INSERT INTO @EES_FINAL
	SELECT
		...
		,CASE WHEN PARCC_GEOM IS NULL THEN '' ELSE PARCC_GEOM
		END AS PARCC_GEOM
		,CASE WHEN PARCC_GEOM IS NULL THEN '' ELSE PARCC_GEOM_PASS_FAIL
		END AS PARCC_GEOM_PASS_FAIL
		,CASE WHEN PARCC_ALG_II IS NULL THEN '' ELSE PARCC_ALG_II
		END AS PARCC_ALG_II
		,CASE WHEN PARCC_ALG_II IS NULL THEN '' ELSE PARCC_ALG_II_PASS_FAIL
		...
		,CASE WHEN SBA_COMBO IS NULL THEN '' ELSE SBA_COMBO_PASS_FAIL
		END AS SBA_COMBO_PASS_FAIL
		,CASE WHEN PARCC_ELA_11 IS NULL THEN '' ELSE PARCC_ELA_11
		END AS 'PARCC_ELA_11'
		,CASE WHEN PARCC_ELA_11 IS NULL THEN '' ELSE PARCC_ELA_11_PASS_FAIL
		...
		,CAST(EES.WRI_ATTEMPTS AS INT) + PARCC_CNTS.WRITING AS 'WRITING ATTEMPTS'
		,CASE WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '') AND ADC.WRITE IS NULL AND ADC.REWR IS NULL AND (EES.WRI_ATTEMPTS < 1 OR EES.WRI_ATTEMPTS IS NULL) THEN 'TAKE PARCC ELA 11'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE IS NULL AND ADC.REWR IS NULL AND (EES.WRI_ATTEMPTS > 0) THEN 'TAKE EOC'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE IS NULL AND ADC.REWR = 'P' THEN 'PASSED ' + '(' + ATN.WRITE +')'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE = 'P' THEN 'PASSED ' + '(' + ATN.WRITE + ')'
...
SELECT * FROM @WR_CNTS
WHERE SIS_NUMBER = '100100858'
SELECT * FROM @SS_COUNT
WHERE SIS_NUMBER = '100100858'
SELECT * FROM @PARCC_CNTS
WHERE SIS_NUMBER = '100101419'
SELECT * FROM @PARCC_ELA_11
WHERE SIS_NUMBER = '100100858'
