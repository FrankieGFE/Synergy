...
EXEC K12INTEL_METADATA.REMOVE_TESTSCORES 'MCLASS_DIBELS_NEXT 001 APS 2015-2016 EOY'	 --'GENERIC SBA-NM 001 APS 2015-2016'		--If the name has changed, used the previous name here.
...
WHERE 1=1
    AND batch.BATCH_NAME in( 'MCLASS_DIBELS_NEXT 001 APS 2015-2016 EOY') --, 'GENERIC HSGA-Composite 001 APS 2015-2016')  --This is the folder structure; needs to be changed if the test is moved.
...
SELECT AUDIT_ID, AUDIT_TIMESTAMP, BUILD_NUMBER, AUDIT_SOURCE, AUDIT_SOURCE_LOCATION, RIGHT(REPLACE(REPLACE(REPLACE(AUDIT_DATA_LINEAGE, CHAR(10), ''), CHAR(13), ''), CHAR(9), ''),100) AS AUDIT_DATA_LINEAGE, AUDIT_BASE_MSG
FROM K12INTEL_AUDIT.RAW_AUDITS
WHERE 1=1
    AND BUILD_NUMBER = 1018
	order by audit_source_location
...
WHERE 1=1
	AND tests.TEST_PRODUCT like 'dibels%'
	AND CONVERT(DATE, scores.SYS_CREATED) = '05/31/2016'
