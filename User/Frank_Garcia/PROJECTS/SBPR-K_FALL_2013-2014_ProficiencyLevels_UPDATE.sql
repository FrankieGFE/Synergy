USE [db_KDPR]
GO
/****** Object:  StoredProcedure [dbo].[test_SBPR_Overall_Scores_sp]    Script Date: 02/05/2014 08:18:01 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE procedure [dbo].[test_SBPR_Overall_Scores_sp] as

DECLARE @AssessmentWindow nvarchar(6)

SET @AssessmentWindow = 'Winter'

UPDATE
	db_KDPR.dbo.Results
SET
	db_KDPR.dbo.Results.MATH_PL = PLS.MATH_PL
	,db_KDPR.dbo.Results.ELA_PL = PLS.ELA_PL
FROM
(		
SELECT
	MATH_.ID_NBR
	...
	,MATH_PL =
		CASE
		WHEN MATH_.MATH_SCORE < 5 THEN 'Not Applicable'
	...
	,ELA_PL =
		CASE
		WHEN LANG_.ELA_SCORE < 5 THEN 'Not Applicable'
...
WHERE
db_KDPR.dbo.Results.fld_ID_NBR = PLS.ID_NBR
AND db_KDPR.dbo.Results.fld_AssessmentWindow = @AssessmentWindow
AND db_KDPR.dbo.Results.fld_Language = PLS.LANG		  
