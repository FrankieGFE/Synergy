...
SELECT  COURSE_ID, COURSE_TITLE, COUNT (*) AS UNIQUE_STUDENT_COUNT
FROM (
SELECT DISTINCT SIS_NUMBER, COURSE_ID, COURSE_TITLE
FROM (
SELECT   
...
FROM
   ...
   [rev].[EPC_SCH_YR_OPT_SCHED] AS [SchedOpt]
	...
	ON 
	[Courses].COURSE_GU = LST.COURSE_GU
