...
ALTER FUNCTION [APS].[STARSPeriodExcusedAsOf](@startDate DATE, @endDate DATE)
RETURNS TABLE
AS
RETURN
...
WHERE
    [abr].[TYPE]= 'EXC' AND ABR.ABBREVIATION = 'RC'
		...
		SELECT 
			   ...
			   FROM 
				(SELECT * FROM APS.BASICSCHEDULE WHERE 
				 ...
				 AND (COURSE_ENTER_DATE IS NULL OR COURSE_ENTER_DATE >= @startDate)
