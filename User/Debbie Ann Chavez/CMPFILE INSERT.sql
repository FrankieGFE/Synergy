...
--removeme.ADD_DATE_TIME_STAMP, REMOVEME.ADD_ID_STAMP,
...
/*
LEFT JOIN 
(SELECT * FROM rev.EPC_STREET WHERE ADD_ID_STAMP = '27CDCD0E-BF93-4071-94B2-5DB792BB735F'
AND rev.EPC_STREET.ADD_DATE_TIME_STAMP > '2016-08-09')
AS REMOVEME

ON
ISNULL(CAST(T1.STREET_LOW_ADDRESS AS INT),'') = ISNULL(REMOVEME.STREET_LOW_ADDRESS,'')
AND ISNULL(T1.STREET_NAME,'') = ISNULL(REMOVEME.STREET_NAME,'')
AND ISNULL(T1.CITY,'') = ISNULL(REMOVEME.CITY,'')
AND ISNULL(T1.STATE,'') = ISNULL(REMOVEME.STATE,'')
AND ISNULL(T1.ZIP_5,'') = ISNULL(REMOVEME.ZIP_5,'')
AND REMOVEME.SCHOOL_YEAR = 2016

WHERE
REMOVEME.ADD_ID_STAMP IS NULL
*/
