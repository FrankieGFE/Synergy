...
SELECT 
	...
	,CASE 
		WHEN HIGHESTCOUNT = 1  THEN 'Heritage' 
		WHEN HIGHESTCOUNT = 2 THEN 'Heritage'
...
(
SELECT 
	STEP1.SIS_NUMBER, STUDENT_GU, EL, SCHOOL_CODE
	,ROW_NUMBER() OVER (PARTITION BY STEP1.SIS_NUMBER ORDER BY STEP1.SIS_NUMBER) AS RN
...
(SELECT * FROM 
(SELECT 
	 SIS_NUMBER, SCH.COURSE_ID, SCH.COURSE_TITLE, COURSE_LEVEL, STATE_COURSE_CODE
