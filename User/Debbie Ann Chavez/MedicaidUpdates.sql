...
DISTINCT
	[District CD]
	   ...
	   ,[Zip Code]
	   ,[Primary Diagnosis CD]
	   , [Secondary Diagnosis CD]
	   , [Tertiary Diagnosis CD]
	   ,[Fourth Diagnosis CD]
	   , [IEP Start Date]
	   , [IEP End Date]
	   ,[PCP Prescription Start Date]
	   , [PCP Prescription End Date]
	   , [PCP Prescription Exempt(E)/In Process(P)]
	   ,[PCP Last Name]
	   ,[PCP First Name]
	   , [Audiology on IEP?]
	   ...
	   , [BH-Mental Health Counselor/Therapist?]
	   , [BH-Psychologist on IEP?]
	   , [BH-School Psychologist on IEP?]
...
(
SELECT 
ROW_NUMBER() OVER (PARTITION BY [Student ID], [FILE] ORDER BY [FILE] DESC) AS RN
,*
FROM (
SELECT
    *
