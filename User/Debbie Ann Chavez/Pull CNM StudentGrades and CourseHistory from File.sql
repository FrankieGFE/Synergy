...
, CASE WHEN LEFT(UNM.FINAL_GRADE,1) != LEFT(GRADES.MARK,1) THEN 'X'
		WHEN LEFT(UNM.FINAL_GRADE,1) != LEFT(CRSHST.MARK,1) THEN 'X'
...
AND GRADES.ORGANIZATION_NAME LIKE ('Central%')
...
AND COALESCE(SUBJ_CODE, '')+ COALESCE(CAST(COURSE_NUM AS VARCHAR), '') = CRSHST.COURSE_ID
