...
SELECT DISTINCT 
	BEP.SCHOOL_CODE 
	,BEP.ORGANIZATION_NAME AS SCHOOL_NAME
	...
	,CRS.STATE_COURSE_CODE AS COURSE_CODE
	,BEP.Course AS COURSE_CONTENT
	,CRS.DEPARTMENT AS [SUBJECT]
...
FROM 
	...
	APS.BilingualModelAndHoursDetailsAsOf('20161201') AS BEP
	INNER JOIN 
	rev.EPC_CRS AS CRS 
	ON
	BEP.CourseID = CRS.COURSE_ID
	INNER JOIN
...
SELECT 
				...
				,DOCUMENT_NUMBER
				,BADGE_NUM
					,STUFF(( SELECT ',' + T2.CREDENTIAL_TYPE AS [text()]
							FROM (SELECT DISTINCT CRD.STAFF_GU, CREDENTIAL_TYPE, DOCUMENT_NUMBER, BADGE_NUM FROM rev.EPC_STAFF_CRD AS CRD
									INNER JOIN rev.EPC_STAFF AS STF ON CRD.STAFF_GU = STF.STAFF_GU
							) AS T2
							WHERE
							T1.STAFF_GU = T2.STAFF_GU
							ORDER BY T2.CREDENTIAL_TYPE
							FOR XML PATH ('')),1,1,'')	AS TYPE_OF_LICENSE
			FROM
				(SELECT DISTINCT CRD.STAFF_GU, CREDENTIAL_TYPE, DOCUMENT_NUMBER, BADGE_NUM FROM rev.EPC_STAFF_CRD AS CRD
									INNER JOIN rev.EPC_STAFF AS STF ON CRD.STAFF_GU = STF.STAFF_GU
							) AS T1

			GROUP BY STAFF_GU, DOCUMENT_NUMBER, BADGE_NUM
...
LEFT JOIN
(
SELECT PRIMARY_TEACHER, LAST_NAME, FIRST_NAME, T.STAFF_GU, SECTION_GU, DOCUMENT_NUMBER AS EA_LICENSE FROM 
...
LEFT JOIN 
(SELECT STAFF_GU, DOCUMENT_NUMBER FROM rev.EPC_STAFF_CRD) AS CRD
ON
P.PERSON_GU = CRD.STAFF_GU
