...
Revision 1 - Include all algebra classes for each student
Revision 2 - Include grade when taking algebra classes 
Revision 3 - Get ELL Exit_Reason and Exit_Date from REV.EPC_STU_PGM_ELL_HIS_NEW (logic rewritten by Debbie)
...
(
SELECT
	ROW_NUMBER() OVER(PARTITION BY [APS ID]  ORDER BY [aps id],  h.SCHOOL_YEAR, h.TERM_CODE) AS RN,
	S.[APS ID],
...
GROUP BY
	...
	--SELECT * FROM ALGEBRACTE1 where [APS ID] = 102768587
,ALGEBRACTE2
AS
(
select
	...
	MAX(CASE WHEN A.RN = 2 THEN GRADE END) AS GRADE_IN_SCHOOL_1,
	MAX(CASE WHEN A.RN = 1 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_1,
	...
	MAX(CASE WHEN A.RN = 2 THEN GRADE END) AS GRADE_IN_SCHOOL_2,
	MAX(CASE WHEN A.RN = 2 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_2,
	...
	MAX(CASE WHEN A.RN = 3 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_3,
	MAX(CASE WHEN A.RN = 3 THEN GRADE END) AS GRADE_IN_SCHOOL_3,
	...
	MAX(CASE WHEN A.RN = 4 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_4,
	MAX(CASE WHEN A.RN = 4 THEN GRADE END) AS GRADE_IN_SCHOOL_4,
	MAX(CASE WHEN A.RN = 5 THEN H_COURSE_TITLE END) AS COURSE_TITLE_5,
	MAX(CASE WHEN A.RN = 5 THEN ALGEBRA_GRADE END) AS ALGEBRA_GRADE_5,
	MAX(CASE WHEN A.RN = 5 THEN TERM_CODE END) AS ALGEBRA_TERM_CODE_5,
	MAX(CASE WHEN A.RN = 5 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_5,
	MAX(CASE WHEN A.RN = 5 THEN GRADE END) AS GRADE_IN_SCHOOL_5,
	MAX(CASE WHEN A.RN = 6 THEN H_COURSE_TITLE END) AS COURSE_TITLE_6,
	MAX(CASE WHEN A.RN = 6 THEN ALGEBRA_GRADE END) AS ALGEBRA_GRADE_6,
	MAX(CASE WHEN A.RN = 6 THEN TERM_CODE END) AS ALGEBRA_TERM_CODE_6,
	MAX(CASE WHEN A.RN = 6 THEN SCHOOL_YEAR END) AS ALGEBRA_YEAR_6,
	MAX(CASE WHEN A.RN = 6 THEN GRADE END) AS GRADE_IN_SCHOOL_6
	--A.SCHOOL_YEAR
...
--select * from ALGEBRACTE2 WHERE [APS ID]	 = 100076843
...
(
SELECT
	ROW_NUMBER() OVER(PARTITION BY [APS ID] ORDER BY [APS ID], term_code) AS RN,
	S.[APS ID],
	h.course_title AS [ H COURSE TITLE],
	H.TERM_CODE,
...
SELECT
	...
	a.GRADE_IN_SCHOOL_4,
	A.ALGEBRA_YEAR_5,
	A.ALGEBRA_TERM_CODE_5,
	A.COURSE_TITLE_5,
	A.ALGEBRA_GRADE_5,
	a.GRADE_IN_SCHOOL_5,
	a.ALGEBRA_YEAR_6,
	A.ALGEBRA_TERM_CODE_6,
	A.COURSE_TITLE_6,
	A.ALGEBRA_GRADE_6,
	a.GRADE_IN_SCHOOL_6,
	E.[ ENGLISH GRADE],
