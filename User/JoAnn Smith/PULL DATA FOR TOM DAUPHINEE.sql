...
declare @SCHOOL_GU UNIQUEIDENTIFIER = '8D50ABCA-A562-481C-B54F-969A5A6B3999'
declare @AsOfDate datetime2 = '2017-10-01'
declare @FallAsOfDate datetime2 = '2016-12-23'
declare @YearEndDate datetime2 = '2017-05-25'
declare @SchoolName nvarchar(50) = 'Roosevelt Middle School'
...
FROM
	DBO.I_READY_FALL_MATH_SCORES_2016 M
INNER JOIN
...
--select * from IREADY_ASSESSMENT_FALL WHERE STUDENT_APS_ID = 970037254
...
FROM
	DBO.I_READY_WINTER_MATH_SCORES_2016 M
INNER JOIN
	APS.SCHEDULEASOF(@AsOfDate) s
ON
	s.SIS_NUMBER = M.STUDENT_APS_ID
...
FROM
	DBO.I_READY_SPRING_MATH_SCORES_2016 M
INNER JOIN
	APS.SCHEDULEASOF(@AsOfDate) s
ON
	s.SIS_NUMBER = M.STUDENT_APS_ID
...
--------------------------------------------------
,CORE_ENGLISH_END_OF_FALL
AS
(
SELECT
	ROW_NUMBER() OVER(PARTITION BY [SIS_NUMBER], H.COURSE_TITLE, H.TERM_CODE ORDER BY [SIS_NUMBER], term_code desc) AS RN,
	s.SIS_NUMBER,
	h.course_title AS FALL_ENG_COURSE_TITLE,
	H.TERM_CODE,
	h.MARK AS [FALL_ENG_GRADE]
from
	STUDENTCTE S
LEFT JOIN
	REV.EPC_STU_CRS_HIS H
...
,CORE_ENGLISH_END_OF_FALL_2
AS
(
SELECT * FROM CORE_ENGLISH_END_OF_FALL WHERE RN =1	
)

--=============================================

,CORE_ENGLISH_END_OF_YEAR
AS
(
SELECT
	ROW_NUMBER() OVER(PARTITION BY [SIS_NUMBER], H.COURSE_TITLE, H.TERM_CODE ORDER BY [SIS_NUMBER], term_code desc) AS RN,
	s.SIS_NUMBER,
	h.course_title AS EOY_ENG_COURSE_TITLE,
	H.TERM_CODE,
	h.MARK AS [EOY_ENG_GRADE]
from
	STUDENTCTE S
LEFT JOIN
	REV.EPC_STU_CRS_HIS H
...
,CORE_ENGLISH_END_OF_YEAR_2
AS
(
SELECT * FROM CORE_ENGLISH_END_OF_YEAR WHERE RN =1	
)
...
SELECT
	...
	ISNULL(F2.TOTAL_UNEXCUSED_FALL,0) AS TOTAL_UNEXCUSED_FALL,
	ISNULL(Y2.TOTAL_UNEXCUSED_YEAR,0) AS TOTAL_UNEXCUSED_YEAR,
	ISNULL(MF.FALL_MATH_COURSE_TITLE, '') AS FALL_MATH_COURSE_TITLE,
	ISNULL(MF.FALL_MATH_GRADE, '') AS FALL_MATH_GRADE,
	ISNULL(MY.EOY_MATH_COURSE_TITLE, '') AS EOY_MATH_COURSE_TITLE,
	ISNULL(MY.EOY_MATH_GRADE, '') AS EOY_MATH_GRADE,
	ISNULL(SF.FALL_SCI_COURSE_TITLE, '') AS FALL_SCIENCE_COURSE_TITLE,
	ISNULL(SF.FALL_SCIENCE_GRADE, '') AS FALL_SCIENCE_GRADE,
	ISNULL(SY.EOY_SCI_COURSE_TITLE, '') AS EOY_SCIENCE_COURSE_TITLE,
	ISNULL(SY.EOY_SCIENCE_GRADE, '') AS EOY_SCIENCE_GRADE,
	ISNULL(EF.FALL_ENG_COURSE_TITLE, '') AS FALL_ENGLISH_COURSE_TITLE,
	ISNULL(EF.FALL_ENG_GRADE, '') AS FALL_ENG_GRADE,
	ISNULL(EY.EOY_ENG_COURSE_TITLE, '') AS EOY_ENG_COURSE_TITLE,
	ISNULL(EY.EOY_ENG_GRADE, '') AS EOY_ENG_GRADE
...
ON
	SP.STUDENT_APS_ID = S.SIS_NUMBER
LEFT JOIN
	CORE_MATH_END_OF_FALL_2 MF
ON
	SP.STUDENT_APS_ID = MF.SIS_NUMBER
LEFT JOIN
	CORE_MATH_END_OF_YEAR_2 MY
ON
	MY.SIS_NUMBER = S.SIS_NUMBER
LEFT JOIN
	CORE_SCIENCE_END_OF_FALL_2 SF
ON
	SF.SIS_NUMBER = S.SIS_NUMBER
LEFT JOIN
	CORE_SCIENCE_END_OF_YEAR_2 SY
ON
	SY.SIS_NUMBER = S.SIS_NUMBER
LEFT JOIN
	CORE_ENGLISH_END_OF_FALL_2 EF
ON
	EF.SIS_NUMBER = S.SIS_NUMBER
LEFT JOIN
	CORE_ENGLISH_END_OF_YEAR_2 EY
ON
	EY.SIS_NUMBER = S.SIS_NUMBER
