...
(
SELECT 
	P.ORGANIZATION_NAME AS SCHOOL_NAME,
	BS.SIS_NUMBER
	,BS.STUDENT_GU
	,P.ENROLLMENT_GRADE_LEVEL
	,BS.GENDER
	,BS.RACE_1
	,BS.RACE_2
	,BS.RACE_3
...
left join
aps.ScheduleDetailsAsOf(@AsOfDate) P
on
...
LEFT JOIN
	REV.EPC_STU ST
ON
	ST.STUDENT_GU = BS.STUDENT_GU
WHERE P.ENROLLMENT_GRADE_LEVEL IN ('100', '110', '120', '130', '140', '150')
and
	p.ORGANIZATION_GU = @SCHOOL_GU
)
...
SELECT
	...
	ISNULL(S.MILITARY_FAMILY_CODE, '') AS MILITARY_FAMILY_CODE,
...
WHERE P.ENROLLMENT_GRADE_LEVEL IN ('160', '170', '180')
and
	p.ORGANIZATION_GU = @SCHOOL_GU
)
--SELECT * FROM STUDENTCTE 
