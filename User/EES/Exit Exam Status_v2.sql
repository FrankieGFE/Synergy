...
INSERT INTO @EES_FINAL
	SELECT
		...
		,CASE WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND (EES.SCIENCE_ATTEMPTS IS NULL OR EES.SCIENCE_ATTEMPTS < 2) THEN 'TAKE SCIENCE SBA'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI IS NULL AND (EES.SCIENCE_ATTEMPTS IS NULL OR EES.SCIENCE_ATTEMPTS < 2) THEN 'TAKE SCIENCE SBA'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI IS NULL AND EES.SCIENCE_ATTEMPTS > 1 THEN 'USE ADC'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI = 'P' AND EES.SCIENCE_ATTEMPTS > 1 THEN 'PASSED SECONDARY' + '(' + ATN.SCI_TEST + ')'
...
WHERE 1 = 1
AND SIS_NUMBER = '970074823'
SELECT * FROM @MRW_CNTS
WHERE 1 = 1
AND SIS_NUMBER = '970074823'
--and SCHOOL = '590'
--AND SIS_NUMBER IN ('970089914','970112129')
