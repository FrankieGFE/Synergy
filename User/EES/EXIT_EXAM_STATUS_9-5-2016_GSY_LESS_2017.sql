...
/* 
* Request by: Rose-Ann Mckernen	
*
* Created: March 2014
*
* Test requirements remaining for graduation
*
* Frank Garcia 
*
* Tables Referenced: 
* Views:  
* Version 2.1 10 SEPT 2017
*  Added ELSE PL to @PARCC_ELA_11 and @PARCC_ALG_II
*  Added GYS and filtered out GSY 217
*/
...
INSERT INTO @MRW_CNTS
SELECT
	SIS_NUMBER
	,SUM (MATH) AS MATH
	,SUM (READING) AS READING
	,SUM (WRITING) AS WRITING
...
SELECT 
	STUD_ID AS SIS_NUMBER
	,[SBA HS Math 2011+] AS MATH
	,[SBA HS Reading 2011+] AS READING
	,'' AS WRITING
...
WHERE
       1 = 1
	   AND TEST_NAME IN ('SBA HS Math 2011+', 'SBA HS READING 2011+', 'SBA HS SCIENCE 2011+')  
       AND SCORE_DESCRIPTION = 'Scale'
) AS T1
PIVOT
	(
	COUNT (SIS_NUMBER) FOR PART_DESCRIPTION IN ([SBA HS Math 2011+], [SBA HS Reading 2011+], [SBA HS Science 2011+])) AS COUNTS 
) AS T2
--WHERE SIS_NUMBER IN ('100015775','100015791','100015817')
GROUP BY SIS_NUMBER
ORDER BY SIS_NUMBER
...
SELECT 
	...
	,SCALE_SCORE
	,PL
FROM APS.StudentTestSBA ('SBA HS MATH 2011+', 'SBA HS MATH 2011+')
...
SELECT 
		STUD_ID AS SIS_NUMBER
       ,CASE WHEN [ELA/L English 11] IS NULL THEN '0'
		     WHEN [ELA/L English 11] = '' THEN '0'
			 WHEN [ELA/L English 11] = '' THEN '0'
		     WHEN [ELA/L English 11] IS NOT NULL THEN [ELA/L English 11]
			 ELSE [ELA/L English 11]
	   END AS TEST
FROM
(
...
FROM
       ...
       ON Person.PERSON_GU = StudentTest.STUDENT_GU
WHERE
       TEST_NAME IN ('PARCC HS ELA/L English 11')
) AS T1
PIVOT
	(COUNT (SIS_NUMBER) FOR PART_DESCRIPTION IN ([PARCC HS Geometry],[PARCC HS Algebra II],[ELA/L English 11])) AS PARCC_CNT
...
INSERT INTO @EOC_SBA_AND_EOC
	SELECT
		...
		,CASE WHEN PARCC_ALG_II IS NULL THEN '' ELSE CASE WHEN PARCC_ALG_II >= 3 THEN 'PASS' ELSE 'FAIL'
		END END AS PASS_FAIL_PARCC_ALG_II
		,PARCC_GEOM AS [PARCC_GEOM]
		,CASE WHEN PARCC_GEOM IS NULL THEN '' ELSE CASE WHEN PARCC_GEOM >= 3 THEN 'PASS' ELSE 'FAIL'
		END END AS PASS_FAIL_PARCC_GEOM
		,PARCC_ELA_11
		,CASE WHEN PARCC_ELA_11 IS NULL THEN '' ELSE CASE WHEN PARCC_ELA_11 >= 3 THEN 'PASS' ELSE 'FAIL'
	...
	SELECT
		...
		--- ADD PARCC PASS/FAIL
		,P_ALG.PL [PARCC_ALG_II] 
		,'' AS PASS_FAIL_PARCC_ALG_II
		,P_GEOM.PL AS [PARCC_GEOM]
		,'' AS PASS_FAIL_PARCC_GEOM
		,P_ELA.PL [PARCC_ELA_11_READ]
		,P_ELA.PL AS [PARCC_ELA_11]
		,'' AS PASS_FAIL_PARCC_ELA_11
		,SS_CNT.TEST AS SS_ATTEMPTS
		,CAST(WR_CNT.TEST AS INT) AS WRI_ATTEMPTS
...
INSERT INTO @EES_FINAL
	SELECT
		...
		END AS SBA_MATH
		,CASE WHEN SBA_MATH IS NULL THEN '' ELSE SBA_MATH_PASS_FAIL
		END AS SBA_MATH_PASS_FAIL
		,CASE WHEN SBA_COMBO IS NULL THEN '' ELSE SBA_COMBO
		END AS SBA_COMBO
		,CASE WHEN SBA_COMBO IS NULL THEN '' ELSE SBA_COMBO_PASS_FAIL
		END AS SBA_COMBO_PASS_FAIL
		...
		,CASE WHEN MRW_CNTS.MATH IS NULL THEN '0' ELSE MRW_CNTS.MATH
		END AS 'MATH ATTEMPTS'
		,CASE 
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND ADC.MATH = 'P' AND MRW_CNTS.MATH > 2 THEN 'PASSED SECONDARY '+ '('+ATN.MATH_TEST+')'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND ADC.MATH != 'P' AND MRW_CNTS.MATH > 2 THEN 'USE ADC'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND ADC.MATH = 'P' AND MRW_CNTS.MATH < 3 THEN 'TAKE PARCC'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND ADC.MATH != 'P' AND MRW_CNTS.MATH < 3 THEN 'TAKE PARCC'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND ADC.MATH != 'P' AND (MRW_CNTS.MATH IS NULL OR MRW_CNTS.MATH < 3) THEN 'TAKE PARCC'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND (MRW_CNTS.MATH IS NULL OR MRW_CNTS.MATH < 3) THEN 'TAKE PARCC'
			  WHEN EES.[MATH REQUIREMENTS] IS NULL AND (MRW_CNTS.MATH IS NULL OR MRW_CNTS.MATH > 2) THEN 'USE ADC'
			  ELSE EES.[MATH REQUIREMENTS]
		END AS 'MATH REQUIREMENTS'
		,CASE WHEN SBA_READING IS NULL THEN '' ELSE SBA_READING
		END AS SBA_READING
		,CASE WHEN SBA_READING IS NULL THEN '' ELSE SBA_READING_PASS_FAIL
		END AS SBA_READING_PASS_FAIL
		,CASE WHEN SBA_COMBO IS NULL THEN '' ELSE SBA_COMBO
		END AS SBA_COMBO
		,CASE WHEN SBA_COMBO IS NULL THEN '' ELSE SBA_COMBO_PASS_FAIL
		END AS SBA_COMBO_PASS_FAIL
		...
		,CASE WHEN MRW_CNTS.READING IS NULL THEN '0' ELSE MRW_CNTS.READING
		END AS 'READING ATTEMPTS'
		,CASE 
		      WHEN (EES.[READING REQUIREMENTS] IS NULL OR EES.[READING REQUIREMENTS] = '') AND (ADC.[READ] = 'P' OR ADC.REWR = 'P') AND MRW_CNTS.READING > 2 THEN 'PASSED SECONDARY ' + '(' + ATN.READ_TEST +')'
			  WHEN (EES.[READING REQUIREMENTS] IS NULL OR EES.[READING REQUIREMENTS] = '') AND ADC.[READ] = 'P' AND MRW_CNTS.READING > 2 THEN 'PASSED SECONDARY' + '(' + ATN.READ_TEST +')'
			  WHEN (EES.[READING REQUIREMENTS] IS NULL OR EES.[READING REQUIREMENTS] = '') AND (MRW_CNTS.READING > 2) THEN 'USE ADC'
			  WHEN (EES.[READING REQUIREMENTS] IS NULL OR EES.[READING REQUIREMENTS] = '') AND (MRW_CNTS.READING IS NULL OR MRW_CNTS.READING < 3) THEN 'TAKE PARCC ELA 11'
			  ELSE EES.[READING REQUIREMENTS]
		END AS 'READING REQUIREMENTS'

		,CASE WHEN MRW_CNTS.WRITING IS NULL THEN '0' ELSE MRW_CNTS.WRITING END AS 'WRITING PARCC ATTEMPTS'
		,CASE WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '') AND ADC.WRITE IS NULL AND ADC.REWR IS NULL AND (MRW_CNTS.WRITING < 1 OR MRW_CNTS.WRITING IS NULL) THEN 'TAKE PARCC ELA 11'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE IS NULL AND ADC.REWR IS NULL AND (MRW_CNTS.WRITING > 0) THEN 'USE ADC'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE IS NULL AND ADC.REWR = 'P' AND MRW_CNTS.WRITING > 0 THEN 'PASSED SECONDARY' + '(' + ATN.WRITE +')'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '')  AND ADC.WRITE = 'P' AND MRW_CNTS.WRITING > 0 THEN 'PASSED SECONDARY' + '(' + ATN.WRITE + ')'
			  WHEN (EES.[WRITING REQUIREMENTS] IS NULL OR EES.[WRITING REQUIREMENTS] = '') AND (MRW_CNTS.WRITING < 1 OR MRW_CNTS.WRITING IS NULL) THEN 'TAKE PARCC ELA 11'
		...
		,CASE WHEN MRW_CNTS.SCIENCE IS NULL THEN '0' ELSE MRW_CNTS.SCIENCE
		END AS 'SCIENCE ATTEMPTS'
		,CASE WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND (MRW_CNTS.SCIENCE IS NULL OR MRW_CNTS.SCIENCE < 2) THEN 'TAKE SCIENCE SBA'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI IS NULL AND (MRW_CNTS.SCIENCE IS NULL OR MRW_CNTS.SCIENCE < 2) THEN 'TAKE SCIENCE SBA'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI IS NULL AND MRW_CNTS.SCIENCE > 1 THEN 'USE ADC'
			  WHEN EES.[SCIENCE REQUIREMENTS] IS NULL AND ADC.SCI = 'P' AND MRW_CNTS.SCIENCE > 1 THEN 'PASSED ' + '(' + ATN.SCI_TEST + ')'
	...
	FROM
		...
		LEFT HASH JOIN
		@EOC_SBA_AND_EOC AS EOC
		ON EES.SIS_NUMBER =  EOC.SIS_NUMBER

		LEFT HASH JOIN
		@MRW_CNTS AS MRW_CNTS
		ON EES.SIS_NUMBER = MRW_CNTS.SIS_NUMBER

SELECT * FROM @EES_FINAL
WHERE 1 = 1
AND (GSY < 2017 OR GSY IS NULL)
--AND SIS_NUMBER = '970026508'
--AND SCHOOL = '596'
--AND SIS_NUMBER IN ('102870334','102727567','103966255','970030244','970062258')
AND ORGANIZATION_GU = @School
