...
THIS SCRIPT READS A SEPERATE FILE FOR 445/JIMMY CARTER ACCOMMODATIONS
IT ALSO CHOOSES THE ELA COURSE ASCENDING INSTEAD OF DESCENDING.
...
FROM 
	...
	*************************************/
	LEFT JOIN 
		...
		APS.PrimaryEnrollmentDetailsAsOf(GETDATE()) AS PRIM
		INNER JOIN 
		(SELECT STUDENT_GU, SIS_NUMBER, COURSE_ID, [TEACHER NAME], SECTION_ID, SCH.ORGANIZATION_YEAR_GU, SCH.STAFF_GU
		,SCH.PERIOD_BEGIN, SCH.PRIMARY_STAFF, SCH.TERM_CODE
		FROM
...
LEFT JOIN 		
		...
		********************************************/
		SELECT * FROM (SELECT 	
			...
			,ROW_NUMBER() OVER (PARTITION BY SCH.STUDENT_GU, SCH.DEPARTMENT ORDER BY SCH.COURSE_ID ASC) AS RN
		...
		INNER JOIN 
		(SELECT STUDENT_GU, SIS_NUMBER, COURSE_ID, [TEACHER NAME], SECTION_ID, SCH.ORGANIZATION_YEAR_GU, SCH.STAFF_GU
		,SCH.PERIOD_BEGIN, SCH.PRIMARY_STAFF, SCH.TERM_CODE, SCH.COURSE_GU, SCH.DEPARTMENT
		FROM
		APS.ScheduleDetailsAsOf(GETDATE()) AS SCH
		INNER HASH JOIN 
		APS.TermDatesAsOf(GETDATE()) AS TRMS
...
UNION ALL
		...
		********************************************/
		...
		INNER JOIN 
		(SELECT STUDENT_GU, SIS_NUMBER, COURSE_ID, [TEACHER NAME], SECTION_ID, SCH.ORGANIZATION_YEAR_GU, SCH.STAFF_GU
		,SCH.PERIOD_BEGIN, SCH.PRIMARY_STAFF, SCH.TERM_CODE, SCH.COURSE_GU, SCH.DEPARTMENT, COURSE_TITLE
		FROM
		APS.ScheduleDetailsAsOf(GETDATE()) AS SCH
		INNER HASH JOIN 
		APS.TermDatesAsOf(GETDATE()) AS TRMS
...
(
		...
		************************************************/
		SELECT * FROM  (
			SELECT 	
					...
					INNER JOIN 
					(SELECT STUDENT_GU, SIS_NUMBER, COURSE_ID, [TEACHER NAME], SECTION_ID, SCH.ORGANIZATION_YEAR_GU, SCH.STAFF_GU
		,SCH.PERIOD_BEGIN, SCH.PRIMARY_STAFF, SCH.TERM_CODE, SCH.COURSE_GU, SCH.DEPARTMENT, sch.COURSE_TITLE
		FROM
		APS.ScheduleDetailsAsOf(GETDATE()) AS SCH
		INNER HASH JOIN 
		APS.TermDatesAsOf(GETDATE()) AS TRMS
		...
		************************************************/
		SELECT * FROM  (
			SELECT 	
					...
					INNER JOIN 
					(SELECT STUDENT_GU, SIS_NUMBER, COURSE_ID, [TEACHER NAME], SECTION_ID, SCH.ORGANIZATION_YEAR_GU, SCH.STAFF_GU
		,SCH.PERIOD_BEGIN, SCH.PRIMARY_STAFF, SCH.TERM_CODE, SCH.COURSE_GU, SCH.DEPARTMENT, SCH.COURSE_TITLE, SCH.SUBJECT_AREA_1
		FROM
		APS.ScheduleDetailsAsOf(GETDATE()) AS SCH
		INNER HASH JOIN 
		APS.TermDatesAsOf(GETDATE()) AS TRMS
  ...
  LEFT JOIN 
           OPENROWSET (
                  ...
                  'SELECT * from PARCCSPRING_445.csv'
                ) AS [PARCC]
