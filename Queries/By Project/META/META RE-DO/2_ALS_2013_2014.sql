...
DECLARE @SchoolYear AS INT = 2014
DECLARE @AsOfDate AS DATETIME = '2013-12-15'
--DONT EVEN NEED THIS ANYMORE: 
DECLARE @120THDAY AS DATETIME = '2013-12-15'

--THIS IS NEEDED FOR ACCESS LATEST EVAL DATE
DECLARE @EVALDate AS DATETIME = '2014-05-22'

-- CHANGE THE DROPOUT FILE NAME!! 

-- CHANGE THE SENIOR START DATE HARDCODED IN SUBSELECT = '2013-10-01'
...
,CASE 
				...
				--WHEN [ENGLISH PROFICIENCY] = '' AND [ELL Eligible] = '' AND [Most Recent Test] NOT IN ('WAPT', 'SCREENER', 'PRE-LAS') AND [Test Date] < '20110701' THEN 'FEPE'
				--WHEN [ENGLISH PROFICIENCY] = '' AND [ELL Eligible]  = ''  AND [Most Recent Test] NOT IN ('WAPT', 'SCREENER', 'PRE-LAS') AND [Test Date] BETWEEN '20110701' AND '20120701' THEN 'FEPM'
				--WHEN [ENGLISH PROFICIENCY] = '' AND [ELL Eligible]  = ''  AND [Most Recent Test] NOT IN ('WAPT', 'SCREENER', 'PRE-LAS') AND [Test Date] BETWEEN '20120701' AND '20130701' THEN 'FEP'
...
SELECT
		...
		,CASE 
			  WHEN ALS.[ENGLISH PROFICIENCY] = 1 THEN 'ELL'
...
FROM 
dbo.BilingualModelHours_2014_80D AS BP
...
FROM 
dbo.BilingualModelHours_2014_80D AS BEPKIDS
GROUP BY ID_NBR
) AS MODELTAGS
...
FROM
APS.ScheduleWithMoreInfoAsOf('2013-12-15') AS SCH
INNER JOIN 
APS.LCEClassesAsOf('2013-12-15') AS LCECLASS
ON
