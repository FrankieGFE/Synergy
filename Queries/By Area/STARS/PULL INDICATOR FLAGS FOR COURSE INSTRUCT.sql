...
-- CHANGE THE FILE FROM STARS - COURSE INSTRUCT, AND THEN FLAG INDICATORS UPDATE COLUMNS
-- CHANGE THE DATE IN VIEWS (READING CURRENT DATE, DOUBLE CHECK)

--THIS ALSO READS A FILE OF CTE COURSES FROM CHRISTINE
...
FROM
            ...
            OPENROWSET (
                  ...
                  'SELECT * from 80DCOURSEINSTRUCT.csv'
                ) AS [FLAGS]
...
ON
COURSE.COURSE_ID = CAST(FLAGS.[APS course] AS VARCHAR)
...
AND SCHOOL.SCHOOL_CODE = CAST(FLAGS.[LOCATION CODE] AS VARCHAR)
