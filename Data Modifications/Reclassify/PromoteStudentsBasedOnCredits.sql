...
FROM 
	 (
SELECT EVERYTHING.*, [NewGrade].VALUE_DESCRIPTION AS NEW_PRETTY_GRADE FROM (
SELECT *,
			CASE WHEN SSY_GRADE = 190 AND EXPECTED_GRADUATION_YEAR >=2018 THEN 200
				 WHEN SSY_GRADE = 200 AND EXPECTED_GRADUATION_YEAR >=2018 THEN 210
				 WHEN SSY_GRADE = 210 AND EXPECTED_GRADUATION_YEAR <2018 THEN 220
			ELSE ''
			END AS NEW_GRADE 
 FROM 
(
