...
BEGIN
	SET @SchRunYearGU = (SELECT YEAR_GU FROM APS.YearDates WHERE EXTENSION = 'R' AND GETDATE() BETWEEN YearDates.START_DATE AND YearDates.END_DATE)
...
BEGIN
	...
	SET @SchRunYearGU = (SELECT YEAR_GU FROM APS.YearDates WHERE EXTENSION = 'R' AND CONVERT(DATE,'08/30/' + CONVERT(VARCHAR(4),YEAR(GETDATE()))) BETWEEN YearDates.START_DATE AND YearDates.END_DATE)
