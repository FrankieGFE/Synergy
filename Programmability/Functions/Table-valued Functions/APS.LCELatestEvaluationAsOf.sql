 ...
 * 
 */
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APS].[LCELatestEvaluationAsOf]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	EXEC('CREATE FUNCTION APS.LCELatestEvaluationAsOf()RETURNS TABLE AS RETURN (SELECT 0 AS DUMMY)')
GO
 ...
 */
ALTER FUNCTION APS.LCELatestEvaluationAsOf(@asOfDate DATETIME)
RETURNS TABLE
AS
RETURN
SELECT
