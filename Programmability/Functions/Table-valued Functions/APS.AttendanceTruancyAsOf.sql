/**
 *  This is the SQL from the Truancy Report created by Don and Shayne
 *	6/1/2016
 *
*/ 
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[APS].[AttendanceTruancyAsOf]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	EXEC('CREATE FUNCTION APS.AttendanceTruancyAsOf() RETURNS TABLE AS RETURN (SELECT 0 AS DUMMY)')
GO

ALTER FUNCTION APS.AttendanceTruancyAsOf(@AsOfDate DATETIME)
RETURNS TABLE
AS
RETURN
