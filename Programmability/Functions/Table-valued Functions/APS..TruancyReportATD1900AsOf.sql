 ...
 */
ALTER FUNCTION [APS].[TruancyReportATD1900AsOf](@AsOfDate DATE)
RETURNS TABLE
AS
RETURN	
...
SELECT
    ...
    ,[Daily].[Unexcused Absence Count] AS [Full-Day Unexcused]
	,[Daily].[Unexcused Absence Count] AS [Total Unexcused]
FROM
    [APS].[DailyUnexcusedAsOf](@AsOfDate) AS [Daily]
...
FROM
    [APS].[PeriodUnexcusedAsOf](@AsOfDate) AS [Period]
...
AND [OrgYear].[YEAR_GU]=(SELECT [YEAR_GU] FROM [rev].[SIF_22_Common_CurrentYearGU])
...
WHERE
	[SSY].[YEAR_GU]=(SELECT [YEAR_GU] FROM [rev].[SIF_22_Common_CurrentYearGU])
...
AND TWO.SCHOOL_YEAR = (SELECT SCHOOL_YEAR FROM rev.SIF_22_Common_CurrentYear)
...
and FIVE.SCHOOL_YEAR = (SELECT SCHOOL_YEAR FROM rev.SIF_22_Common_CurrentYear)
...
AND TEN.SCHOOL_YEAR = (SELECT SCHOOL_YEAR FROM rev.SIF_22_Common_CurrentYear)

-- THIS IS ONLY NEEDED FOR THE SSRS REPORT:  
