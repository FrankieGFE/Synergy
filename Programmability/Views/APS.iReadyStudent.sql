...
ALTER VIEW APS.iReadyStudent AS
...
FROM
	(
	SELECT STUDENT_GU, STUDENT_SCHOOL_YEAR_GU FROM 	APS.PrimaryEnrollmentsAsOf(GETDATE())  AS [Enrollments]

	UNION

	SELECT STUDENT_GU, STUDENT_SCHOOL_YEAR_GU FROM APS.NonPrimaryEnrollmentsAsOf (GETDATE()) AS NONPR
	)
	AS [Enrollments]
...
SELECT
	...
	,[Student SIS ID]
	,[Student Number]
	,REPLACE([First Name],'"','''') AS [First Name]
	,REPLACE([Last Name],'"','''') AS [Last Name]
	,[Grade Level]
	,[User Name]
	,[Password]
	,[DOB]
...
SELECT DISTINCT
	...
	,[STUDENT].[SIS_NUMBER] AS [Student SIS ID]
	,[STUDENT].[SIS_NUMBER] AS [Student Number]
	,REPLACE([STUDENT].[FIRST_NAME],',','') AS [First Name]
	,REPLACE([STUDENT].[LAST_NAME],',','') AS [Last Name]
	,CASE 
		WHEN [ENROLLMENTS].[GRADE] = 'K' THEN '0'
		WHEN [ENROLLMENTS].[GRADE] IN ('C1','C2','C3','C4','T1','T2','T3','T4') THEN '12' 
		ELSE [ENROLLMENTS].[GRADE] END AS [Grade Level]
	...
	,ROW_NUMBER() OVER (PARTITION BY [STUDENT].[SIS_NUMBER] ORDER BY [STUDENT].[SIS_NUMBER], EXCLUDE_ADA_ADM) AS RN
...
--	)
	...
	--AND [STUDENT].[STATE_STUDENT_NUMBER] IN ('100505296')
	
) AS [STUFF]

WHERE
	[RN] = 1
	 --[Student SIS ID] = '104431655'