...
(
   ...
   FROM rev.EPC_STU                    stu
        ...
        JOIN rev.REV_YEAR              yr   ON yr.YEAR_GU = oyr.YEAR_GU
												  AND oyr.YEAR_GU=(SELECT [YEAR_GU] FROM [APS].[YearDates] WHERE CAST(GETDATE() AS DATE) BETWEEN [START_DATE] AND [END_DATE] AND EXTENSION != 'R')
...
FROM rev.EPC_STU_SCH_YR             ssy    
     ...
     JOIN rev.REV_YEAR              y      ON y.YEAR_GU                   = ssy.YEAR_GU 
                                              AND y.SCHOOL_YEAR           = 2016 --(select SCHOOL_YEAR from rev.SIF_22_Common_CurrentYear)
											  AND y.EXTENSION             = 'R'
