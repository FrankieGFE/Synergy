 ...
 *
 * This is the SchoolMessenger pull for Daily Attendance.
 */
 
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[SchoolMessenger].[DailyAttendance]'))
	EXEC ('CREATE VIEW SchoolMessenger.DailyAttendance AS SELECT 0 AS DUMMY')
GO

ALTER VIEW SchoolMessenger.DailyAttendance AS
SELECT 
	S.SIS_NUMBER
	,SCH.SCHOOL_CODE
	,CONVERT(VARCHAR(10) , ESAD.ABS_DATE, 112) AS [ABSENCE DATE]
...
JOIN rev.EPC_SCH				  AS SCH	ON OY.ORGANIZATION_GU				   = SCH.ORGANIZATION_GU
