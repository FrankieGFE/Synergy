...
ALTER VIEW [SchoolMessenger].[StudentContact] AS
...
(
   ...
   JOIN rev.REV_YEAR              AS Y   ON Y.YEAR_GU               = SY.YEAR_GU 
										AND (y.YEAR_GU IN(SELECT [YEAR_GU] FROM [rev].[REV_YEAR] WHERE [SCHOOL_YEAR]=2016 AND [EXTENSION] IN ('N', 'R')) --[APS].[YearDates] WHERE CAST(GETDATE() AS DATE) BETWEEN [START_DATE] AND [END_DATE])
...
, CASE WHEN EXTENSION IN ('N', 'S') THEN 'summer' ELSE '' END AS [Some SCH Attendance]
...
left join RaceCodes            as r5     on r5.STUDENT_GU           = s.STUDENT_GU and r5.rn =5
-------------------
WHERE SCH.SCHOOL_CODE IS NOT NULL /*AND ([SCH].[SCHOOL_CODE] BETWEEN '400' AND '599'
      OR SCH.SCHOOL_CODE IN ('227','237','249','250','261','280','327','364'))*/*/
