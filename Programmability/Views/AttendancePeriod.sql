 ...
 *
 * This view pulls Period Attendance for current regular school year - takes about 34 seconds to run for 1,989,614 records (so far).
 * This EXLCUDES TARDIES
 */
 
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[APS].[AttendancePeriod]'))
	EXEC ('CREATE VIEW APS.AttendancePeriod AS SELECT 0 AS DUMMY')
GO

ALTER VIEW APS.AttendancePeriod AS


SELECT SIS_NUMBER, ORG.ORGANIZATION_NAME, ESAD.ABS_DATE, BELL_PERIOD, ECAR.ABBREVIATION  AS [ABSENCE CODE]
,S.STUDENT_GU, SSY.STUDENT_SCHOOL_YEAR_GU, ESAP.PERIOD_ATTEND_GU, ESAD.DAILY_ATTEND_GU
...
INNER JOIN rev.REV_YEAR                 AS Y      ON Y.YEAR_GU                           = ORGYR.YEAR_GU 
                       --This view only runs for the current school year and Regular Extension
                        ...
                        AND Y.EXTENSION  = 'R'
WHERE
	ISNUMERIC(ECAR.ABBREVIATION) != 1 AND ECAR.ABBREVIATION != 'T'
