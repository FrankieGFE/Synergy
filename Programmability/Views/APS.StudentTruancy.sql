...
SELECT 
  ...
  ,rev.UD_STU.SCHOOL_REFERRAL AS [School Referral]
  ...
  ,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(CONVERT(VARCHAR(MAX),TRUANCYLOG.NOTES), CHAR(9), ' '), CHAR(13), ' '), CHAR(10), ' '))) AS [Notes]
  ...
  ,TRUANCYLOG.DATE AS [Truancy Log Date]
...
From
  ...
  rev.UD_STU
  ON
  rev.EPC_STU.STUDENT_GU = rev.UD_STU.STUDENT_GU
...
--Lookup Tables
	...
	rev.EPC_STU.SIS_NUMBER 
	  ...
	  ,rev.UD_STU.SCHOOL_REFERRAL
	  ,MOSTRECENTENROLL.ORGANIZATION_GU 
	  ,CONTACTTYPE.VALUE_DESCRIPTION 
	  ,CONTACT.VALUE_DESCRIPTION
	  ,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(CONVERT(VARCHAR(MAX),TRUANCYLOG.NOTES), CHAR(9), ' '), CHAR(13), ' '), CHAR(10), ' '))) 
